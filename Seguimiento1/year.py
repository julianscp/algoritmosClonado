...
OUT_PNG = OUT_DIR / "ranking_algoritmos_año.png"
...
def get_year(entry: str) -> int | None:
    m = RE_YEAR.search(entry)
    if not m: return None
    val = (m.group(1) or m.group(2) or "").strip()
    m2 = re.search(r'\b(19|20)\d{2}\b', val)
...
def main():
    ...
    years = [y for y in (get_year(e) for e in entries) if y is not None]
    data = years[:min(SAMPLE_N, len(years))]
    gold = sorted(data)

    # Orden del seguimiento
    algs_in_order = [
    ...
    # 1) ÚNICA salida ordenada (primeros 50 elementos)
    print("\nPrimeros 50 años ordenados (asc):")
    print(gold[:50])

    # 2) Ranking por tiempo (rápido → lento)
    results = []
    ...
    results.sort(key=lambda t: t[1])  # por tiempo asc
    ...
    plt.ylabel("Tiempo (s)")
    plt.title(f"Ranking de algoritmos (n={len(data)} años)")
